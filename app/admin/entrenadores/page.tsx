...
export default function AdminEntrenadoresPage() {
  ...
  const fetchEntrenadores = async () => {
    try {
      ...
      if (!response.ok) throw new Error("No se pudieron cargar los entrenadores")
  ...
  const handleOpenDialog = (entrenador?: Entrenador) => {
    ...
    if (entrenador) {
      ...
      setImagePreview(entrenador.FotoDemo || "")
      setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.telefono && !validatePhone(formData.telefono)) {
      toast({
        variant: "warning",
        title: "Atención",
        description: "Ingresa un teléfono válido en formato (+56) 9 xxxx xxxx.",
      })
      return
    }

    // (Opcional) Toast info mientras procesa
    toast({
      variant: "info",
      title: "Procesando",
      description: editingEntrenador ? "Actualizando entrenador..." : "Creando entrenador...",
    })
      ...
      try {
        ...
        if (!validationResult.valid) {
          toast({
            variant: "error",
            title: "Error",
            description: (validationResult.errors || ["Error de validación"]).join(" • "),
          })
          return
        }
      } catch (error) {
        ...
        toast({
          variant: "error",
          title: "Error",
          description: "Error al validar la cuenta.",
        })
        return
      }
    }
    ...
    try {
      let fotoDemo = editingEntrenador?.FotoDemo || ""
      const fotoURL = editingEntrenador?.FotoURL || ""
      ...
      if (imageFile) {
        ...
        if (uploadResponse.ok) {
          ...
          toast({
            variant: "error",
            title: "Error",
            description: "Error al subir la imagen: " + (errorData.error || "Error desconocido"),
          })
          return
        }
      }
      ...
      const body = editingEntrenador
        ? {
            ...formData,
            entrenadorID: editingEntrenador.EntrenadorID,
            estado: editingEntrenador.Activo,
            fotoDemo,
            fotoURL,
          }
        : { ...formData, fotoDemo, fotoURL }
      ...
      if (!response.ok) {
        ...
        throw new Error(error.error || "Error al guardar entrenador")
      }

      const result = await response.json().catch(() => ({}))

      // Si creaste y vino password temporal:
      if (!editingEntrenador && result.tempPassword) {
        toast({
          variant: "success",
          title: "Entrenador creado",
          description: "Se generó una contraseña temporal. Revisa el mensaje y compártela de forma segura.",
        })
        // Mantengo el alert con credenciales porque suele necesitar copy/paste.
        // Si quieres, lo cambiamos por un Dialog.
        alert(
        ...
        toast({
          variant: "success",
          title: "Guardado",
          description: editingEntrenador ? "Entrenador actualizado exitosamente." : "Entrenador creado exitosamente.",
        })
      }
      ...
      toast({
        variant: "error",
        title: "Error",
        description: error?.message || "Error al guardar entrenador.",
      })
    }
  }

  const handleDelete = async (id: number) => {
    const ok = confirm("¿Estás seguro de eliminar este entrenador?")
    if (!ok) {
      toast({
        variant: "info",
        title: "Acción cancelada",
        description: "No se eliminó el entrenador.",
      })
      return
    }
    ...
    toast({
      variant: "info",
      title: "Procesando",
      description: "Eliminando entrenador...",
    })

    try {
      const response = await fetch(`/api/admin/entrenadores?entrenadorID=${id}`, { method: "DELETE" })
      if (!response.ok) {
        const err = await response.json().catch(() => ({}))
        throw new Error(err.error || "No se pudo eliminar el entrenador")
      }
      ...
      toast({
        variant: "error",
        title: "Error",
        description: error?.message || "Error al eliminar entrenador.",
      })
    }
  }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {filteredEntrenadores.length === 0 ? (
            <Card className="col-span-full">
              <CardContent className="p-8 text-center text-muted-foreground">No se encontraron entrenadores</CardContent>
            </Card>
          ) : (
            <>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-2">
                <Label>Foto de Demostración</Label>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setFormData((p) => ({ ...p, certificaciones: e.target.value }))}
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) => setFormData((p) => ({ ...p, biografia: e.target.value }))}
