...
export default function SocioHorariosPage() {
  ...
  const formatearHora = (hora: string) => {
    let horaLimpia = hora
    // Si viene como timestamp completo "1970-01-01T06:00:00", extraer solo la hora
    if (hora.includes("T")) {
  ...
  return (
    <DashboardLayout role="Socio">
      <div className="space-y-6">
        <div className="rounded-xl bg-white p-8 shadow-sm border-l-8 border-[#B1121A]">
          <h1 className="text-3xl font-bold text-[#B1121A]">Horarios del Gimnasio</h1>
          <p className="text-[#B1121A] m-1">Consulta los horarios de apertura y cierre</p>
        </div>

        {estadoActual && (
          <Card className={`pt-0 ${estadoActual.abierto ? "border-green-500 bg-green-50/50" : "border-red-500 bg-red-50/50" }`}>
            <CardContent>
